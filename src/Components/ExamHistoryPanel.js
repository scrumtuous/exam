				  ...
				  <div id="historyJumper">
{
  this.getExamHistory().exams.map((exam, i) => {
    // Map each question to a button span
    let buttons = exam.questions.map((value, index) => {
      return <span key={index}>{this.getButton(value)}</span>;
    });

    // Extract title and question count from exam.name
    const [rawTitle, ids] = exam.name.split("~");
    const title = rawTitle
      .replace(/-/g, " ")
      .replace(/\b\w/g, c => c.toUpperCase());
    const questionCount = ids ? ids.split(",").length : 0;
    const label = `${title} (${questionCount} questions)`;

    // Add label, line break, and horizontal rule
    buttons.unshift(<br key={`br-${i}`} />);
    buttons.unshift(<strong key={`title-${i}`}>{label}</strong>);
    buttons.push(<hr key={`hr-${i}`} />);
...
var result = 'btn btn-warning btn-sm';
		...
		if (!correct) {
			result = <span>
<button className="btn" style={{ background: 'linear-gradient(45deg, #f528cb, #6831e3)', border: 'none' }}>
...
&nbsp;
		...
		if (correct){
			result = <span>
<button className="btn" style={{ background: 'linear-gradient(45deg, #ffffff, #6831e3)', border: 'none' }}>		
